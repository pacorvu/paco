...
const isPlacementOffer = (offer) => {
  ...
  const isInternshipCumFulltime = 
    ...
    (jobType.includes('full time') && jobType.includes('ppo'));
...
export const getOverallStats = async (req, res) => {
  try {
    ...
    // Get placement seeking students
    let placementSeekingQuery = supabase
      .from('students')
      .select('*', { count: 'exact', head: true })
      .ilike('placement_declaration_status', 'yes');
    if (schoolUSNs !== null && schoolUSNs.length > 0) {
      placementSeekingQuery = placementSeekingQuery.in('usn', schoolUSNs);
    }
    const { count: totalPlacementSeekingStudents, error: placementSeekingError } = await placementSeekingQuery;
    if (placementSeekingError) throw placementSeekingError;
    ...
    allOffers?.forEach(offer => {
      ...
      if ((jobType.includes('internship') && jobType.includes('cum') && jobType.includes('full time')) ||
        ...
        totalInternshipCumFulltime++;
                 ...
                 !jobType.includes('internship') && !jobType.includes('ppo')) {
        totalPlaced++;
    ...
    res.status(200).json({
      ...
      data: {
        ...
        totalPlacementSeekingStudents: totalPlacementSeekingStudents || 0
      }
    });
  } catch (error) {
...
export const getSchoolDistribution = async (req, res) => {
  try {
    ...
    // Get all students with their schools and placement declaration status
    const { data: studentsData, error: studentsError } = await supabase
      .from('students')
      .select('usn, school, placement_declaration_status');
    ...
    const schoolCounts = {};
    const schoolPlacementSeekingCounts = {};
    ...
    studentsData?.forEach(student => {
      ...
      if (!schoolPlacementCounts[school]) {
        schoolPlacementSeekingCounts[school] = 0;
        schoolPlacementCounts[school] = new Set();
        schoolInternshipCounts[school] = new Set();
        schoolAnyOfferCounts[school] = new Set();
      }
      
      const status = (student.placement_declaration_status || '').toLowerCase();
      if (status === 'yes') {
        schoolPlacementSeekingCounts[school] = (schoolPlacementSeekingCounts[school] || 0) + 1;
      }
    ...
    const distribution = Object.keys(schoolCounts).map(school => ({
      ...
      placementSeeking: schoolPlacementSeekingCounts[school] || 0,
...
export const getPlacementBySchool = async (req, res) => {
  try {
    ...
    const { data: offersData, error: offersError } = await supabase
      .from('job_offers')
      .select('usn, final_interview_status, offer_letter_status, job_type');
    ...
    const schoolStatsCount = {}; // Track job types per school
    const placedUSNs = new Set();
    ...
    offersData?.forEach(offer => {
      ...
      // Initialize stats for school if needed
      if (!schoolStatsCount[school]) {
        schoolStatsCount[school] = {
          fullTime: 0,
          internship: 0,
          ppo: 0
        };
      ...
      const isInternshipCumFulltime = 
        (jobType.includes('internship') && jobType.includes('cum') && jobType.includes('full time')) ||
        (jobType.includes('full time') && jobType.includes('ppo'));
      
      if (isInternshipCumFulltime) {
        schoolStatsCount[school].ppo++;
      } else if ((jobType.includes('full time') || jobType.includes('fulltime')) && 
                 !jobType.includes('internship') && !jobType.includes('ppo')) {
        schoolStatsCount[school].fullTime++;
      } else {
        // Everything else is counted as internship or others
        schoolStatsCount[school].internship++;
      }
    ...
    const schoolStats = Array.from(allSchools).map(school => {
      ...
      const typeStats = schoolStatsCount[school] || { fullTime: 0, internship: 0, ppo: 0 };
      ...
      return {
        ...
        fullTime: typeStats.fullTime,
        internship: typeStats.internship,
        ppo: typeStats.ppo
      };
    });
...
export const createCompany = async (req, res) => {
  try {
    const supabase = getSupabaseClient();
    const { company_name, description, company_type, website, linkedin, company_logo_link } = req.body;
...
export const updateCompany = async (req, res) => {
  try {
    const supabase = getSupabaseClient();
    const oldCompanyName = decodeURIComponent(req.params.companyName);
    const { company_name, description, company_type, website, linkedin, company_logo_link } = req.body;
    ...
    if (company_name && company_name !== oldCompanyName) {
      const { data: existing, error: fetchError } = await supabase
        .from('companies')
        .select('company_name')
        .eq('company_name', company_name)
      ...
      if (fetchError) throw fetchError;
      if (existing) {
        return res.status(400).json({ success: false, message: 'New company name already exists' });
      }
    }
    ...
    if (company_type !== undefined) updates.company_type = company_type;
    if (website !== undefined) updates.website = website;
    if (linkedin !== undefined) updates.linkedin = linkedin;
    if (company_logo_link !== undefined) updates.company_logo_link = company_logo_link;
