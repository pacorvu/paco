...
const Dashboard = () => {
  ...
  const fetchPlacementStats = async (schoolFilter = null) => {
    try {
      ...
      const overallStatsData = {
        ...
        totalPlacementSeekingStudents: overallData.totalPlacementSeekingStudents || 0,
  ...
  const placementBySchool = schoolStats.map(stat => {
    return {
      ...
      fullTime: stat.fullTime || 0,
      internship: stat.internship || 0,
      ppo: stat.ppo || 0,
      percent: parseFloat((stat.offerPercent || 0).toFixed(2))
    };
  ...
  const displayStats = {
    ...
    totalPlacementSeekingStudents: overallStats?.totalPlacementSeekingStudents || 0,
  ...
  const placementSeekingTotal = 723;
  ...
  const totalStudentsSum = displayPlacementBySchool.reduce((sum, row) => sum + (row.totalStudents || 0), 0);
  const totalFullTime = displayPlacementBySchool.reduce((sum, row) => sum + (row.fullTime || 0), 0);
  const totalInternship = displayPlacementBySchool.reduce((sum, row) => sum + (row.internship || 0), 0);
  const totalPPO = displayPlacementBySchool.reduce((sum, row) => sum + (row.ppo || 0), 0);
  const totalOffersSum = displayPlacementBySchool.reduce((sum, row) => sum + (row.totalOffers || 0), 0);
  
  // Recalculate percentage based on visible offers and fixed placement seeking count (or should it be visible students?)
  // Keeping consistent with previous logic: Total Offers / 723
  const footerOfferPercent = placementSeekingTotal > 0 ? (totalOffersSum / placementSeekingTotal) * 100 : 0;
                ...
                <Box>
                  <HStack justify="space-between" align="center" mb={4}>
                    <Heading as="h2" fontSize="xl" fontWeight="bold" color="gray.700">
                      ...
                      {/* Total Students who opted for placements school-wise */}
                    </Heading>
                    {isSuperAdmin && multipleSchoolsSelected && (
                  ...
                  <SimpleGrid columns={{ base: 2, sm: 4, lg: 8 }} spacing={4} mb={8}>
                    {schoolDistribution.length > 0 ? (
                      schoolDistribution.map((school) => {
                        ...
                        // const countToUse = school.placementSeeking || 0;
                        // const maxCount = Math.max(...schoolDistribution.map(s => s.placementSeeking || 0));
                        const countToUse = school.total || 0;
                        const maxCount = Math.max(...schoolDistribution.map(s => s.total || 0));
                        const isTopSchool = countToUse === maxCount && maxCount > 0;
                ...
                >
                  ...
                  >
                    {hiringPartners.length > 0 ? (() => {
                      ...
                      const scrollDuration = Math.min(150, Math.max(24, uniqueCompanies.length * 1.25));
                    ...
                    <TableContainer overflowX="auto">
                      <Table variant="simple" size="md">
                        <Thead bgGradient="linear(to-r, #172e36, #1e3a47)" borderBottom="2px solid" borderColor="#1e3a47">
                          <Tr>
                            <Th color="white" fontSize="sm" fontWeight="800" textTransform="uppercase" letterSpacing="wider" py={4} px={6}>
                              School
                            </Th>
                            <Th isNumeric color="white" fontSize="sm" fontWeight="800" textTransform="uppercase" letterSpacing="wider" py={4} px={6}>
                              Total Students
                            </Th>
                            <Th isNumeric color="white" fontSize="sm" fontWeight="800" textTransform="uppercase" letterSpacing="wider" py={4} px={6}>
                              Full time Job
                        ...
                        <Tbody>
                          {displayPlacementBySchool.length > 0 ? (
                            displayPlacementBySchool.map((row, index) => (
                              ...
                              >
                                ...
                                <Td isNumeric px={6} py={5} fontSize="md" fontWeight="bold" color="gray.700">
                                  {row.totalStudents}
                                </Td>
                                <Td isNumeric px={6} py={5} fontSize="md" fontWeight="bold" color="blue.600">
                                  {row.fullTime}
                                </Td>
                                <Td isNumeric px={6} py={5} fontSize="md" fontWeight="bold" color="purple.600">
                                  {row.internship}
                                </Td>
                                <Td isNumeric px={6} py={5} fontSize="md" fontWeight="bold" color="orange.600">
                                  {row.ppo}
                                </Td>
                                <Td isNumeric px={6} py={5} fontSize="md" fontWeight="800" color="green.700">
                                  {row.totalOffers}
                                </Td>
                                <Td isNumeric px={6} py={5} fontSize="md" fontWeight="medium">
                                  <Badge
                                    fontSize="sm"
                                    fontWeight="bold"
                                    px={3}
                          ...
                          >
                            <Td px={6} py={5} fontSize="md" textTransform="uppercase" fontWeight="800">
                              Total
                            </Td>
                            <Td isNumeric px={6} py={5} fontSize="md" fontWeight="800">
                              <Box textAlign="right">
                                <Text>{totalStudentsSum}</Text>
                                <Text fontSize="xs" color="gray.400" fontWeight="normal">(opted: {placementSeekingTotal})</Text>
                              </Box>
                            </Td>
                            <Td isNumeric px={6} py={5} fontSize="md" fontWeight="800" color="blue.200">
                              {totalFullTime}
                            </Td>
                            <Td isNumeric px={6} py={5} fontSize="md" fontWeight="800" color="purple.200">
                              {totalInternship}
                            </Td>
                            <Td isNumeric px={6} py={5} fontSize="md" fontWeight="800" color="orange.200">
                              {totalPPO}
                            </Td>
                            <Td isNumeric px={6} py={5} fontSize="lg" fontWeight="900" color="green.300">
                              {totalOffersSum}
                            </Td>
                            <Td isNumeric px={6} py={5} fontSize="md" fontWeight="800">
                              {footerOfferPercent.toFixed(2)}%
                            </Td>
                          </Tr>
                        </Tbody>
                      </Table>
